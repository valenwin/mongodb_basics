...
if __name__ == "__main__":
    ...
    all_products = items_collection.find({})
    ...
    count = items_collection.count_documents({"category": category_to_count})
    print(f"There are {count} products in the category '{category_to_count}'.")

    # 4 - Count the number of unique categories
    unique_categories = items_collection.distinct("category")
    ...
    unique_producers = items_collection.distinct("producer")
    ...
    items_collection.update_many(query, update_operation)
